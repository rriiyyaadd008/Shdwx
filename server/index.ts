...
// CORS configuration for universal hosting
app.use(cors({
...
(async () => {
  ...
  // Use hosting configuration for universal deployment
  ...
  server.listen({
    ...
    log(`serving on ${host}:${port} (${config.hosting.environment})`);
    log(`Base URL: ${config.hosting.baseUrl}`);
  });
})();
